...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM gunun_mansetleri WHERE id = ?");
...
if (isset($_POST['save'])) {
    ...
    if ($id > 0) {
        if ($resimPath) {
            $stmt = $conn->prepare("UPDATE gunun_mansetleri SET baslik=?, aciklama=?, detay=?, resim=? WHERE id=?");
            $stmt->bind_param('ssssi', $baslik, $aciklama, $detay, $resimPath, $id);
        } else {
            $stmt = $conn->prepare("UPDATE gunun_mansetleri SET baslik=?, aciklama=?, detay=? WHERE id=?");
            $stmt->bind_param('sssi', $baslik, $aciklama, $detay, $id);
        }
        $stmt->execute();
        $stmt->close();
    } else {
        $stmt = $conn->prepare("INSERT INTO gunun_mansetleri (baslik, aciklama, detay, resim) VALUES (?,?,?,?)");
...
if (isset($_GET['delete_haber'])) {
    ...
    $stmt = $conn->prepare('DELETE FROM gunun_mansetleri WHERE id = ?');
...
// Haberleri çek
$haberler = $conn->query("SELECT * FROM gunun_mansetleri ORDER BY tarih DESC");
if (!$haberler) {
...
<!-- Haber Ekle/Düzenle -->
<section class="card">
    <h2><?= $editHaber ? "Manşet Düzenle (#{$editHaber['id']})" : "Yeni Manşet Ekle" ?></h2>
